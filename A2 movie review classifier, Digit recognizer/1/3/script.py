...
def calculateProbability(label, testIth, laplace):
    ...
    for word in testIth.keys():
        if word in vocabulary:
            ...
            wordPositive = word in positive_words
            wordNegative = word in negative_words
            if (wordPositive and label >= 7) or (wordNegative and label <= 4):
                if (wordPositive and label >= 7):
                    ans += (e1 + (label - 7) * e2) * occInLabel
                else:
                    ans += (e1 + (4 - label) * e2) * occInLabel
            if (wordPositive and label <= 4) or (wordNegative and label >= 7):
                if (wordPositive and label <= 4):
                    ans -= (e1 + (4 - label) * e2) * occInLabel
                else:
                    ans -= (e1 + (label - 7) * e2) * occInLabel
...
if __name__ == "__main__":
    ...
    # pickleDump('negative_words', negative_words)
    negative_words = pickleLoad('negative_words')
    # pickleDump('positive_words', positive_words)
    positive_words = pickleLoad('positive_words')
